...
def search_similar_vectors(query_vector, top_k=10):
    ...
    search_params = {
        ...
        "params": {
            "nprobe": 10
        }  # 搜索参数，nprobe 控制搜索时查找的聚类数量，影响召回率和性能
        ...
        if similar_results:
            ...
            for idx, result in enumerate(similar_results, 1):
                ...
                similarity_percent = max(
                    0, (1 - result['distance'])) * 100  # 确保百分比不为负
                # 打印每个相似结果的信息：排名、ID、文件名、距离和计算出的相似度百分比
                print(
                    f"第{idx}个相似图像 - ID: {result['id']}, 文件名: {result['filename']}, 距离: {result['distance']:.4f}, 相似度: {similarity_percent:.2f}%"
                )
        else:
            # 如果没有找到相似结果，打印提示信息
            print("未找到相似图像。")
